...
def main():
    ...
    chrom, shift, driver, signed = chrom[keep], shift[keep], driver[keep], signed[keep]
    ...
    print(f"[{'all' if a.all_regions else 'opening-only'}] pca_k={a.pca_k}  "
          f"held-out positives={int(ev_lab.sum())} over {n_genes} genes  "
          f"negatives={int((~ev_lab).sum())}")
    print(f"AUROC head (leave-one-gene-out) : {au_head:.3f}")
